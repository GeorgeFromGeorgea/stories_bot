...
def init_db():
    ...
    with sqlite3.connect(DB_NAME) as conn:
        ...
        # Таблица использования медиа сегодня (чтобы не повторять одну и ту же комбинацию (медиа + подпись) в один день)
...
# --- Посты ---
def add_post(post_type: str, post_time: str, media_id: int, caption: str, signature_id: int = 0) -> int:
    """Добавить запланированный пост. Возвращает ID."""
    with sqlite3.connect(DB_NAME) as conn:
        cur = conn.execute("""INSERT INTO scheduled_posts (post_type, post_time, media_id, caption, signature_id)
            VALUES (?, ?, ?, ?, ?)""", (post_type, post_time, media_id, caption, signature_id))
        conn.commit()
        return cur.lastrowid
...
def update_post(post_id: int, post_type: str = None, post_time: str = None, caption: str = None, signature_id: int = None) -> bool:
    """Редактировать пост."""
    fields = []
    values = []
    if post_type:
...
# --- Подписи (signatures) ---
def add_signature(text: str, name: str = "") -> int:
    with sqlite3.connect(DB_NAME) as conn:
        cur = conn.execute("INSERT INTO signatures (name, text) VALUES (?, ?)", (name, text))
        conn.commit()
        return cur.lastrowid

def get_signature(signature_id: int) -> Optional[Dict]:
    with sqlite3.connect(DB_NAME) as conn:
        conn.row_factory = sqlite3.Row
        cur = conn.execute("SELECT * FROM signatures WHERE id=? AND is_active=1", (signature_id,))
        row = cur.fetchone()
        return dict(row) if row else None
...
def get_all_signatures() -> List[Dict]:
    with sqlite3.connect(DB_NAME) as conn:
        conn.row_factory = sqlite3.Row
        cur = conn.execute("SELECT * FROM signatures WHERE is_active=1 ORDER BY id")
        return [dict(row) for row in cur.fetchall()]

def update_signature(signature_id: int, text: str = None, name: str = None, is_active: int = None) -> bool:
    fields = []
    values = []
    if text is not None:
        fields.append("text=?")
    ...
    if not fields:
        return False
    values.append(signature_id)
    query = f"UPDATE signatures SET {', '.join(fields)} WHERE id=?"
    try:
        with sqlite3.connect(DB_NAME) as conn:
            conn.execute(query, values)
            conn.commit()
...
def delete_signature(signature_id: int) -> bool:
    return update_signature(signature_id, is_active=0)
